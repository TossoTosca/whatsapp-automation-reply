...
<template>
  <div
    class="w-72 h-[520px] bg-black rounded-[40px] p-3 shadow-2xl border-4 border-zinc-800 relative ring-1 ring-zinc-700 select-none">
    <div class="absolute top-5 left-1/2 -translate-x-1/2 w-28 h-4 bg-black rounded-full z-20"></div>

    <div
      class="w-full h-full bg-zinc-900 rounded-[32px] overflow-hidden flex flex-col justify-between p-4 relative pt-8">
      <div class="flex items-center space-x-2 pb-2 border-b border-zinc-800/60">
        <div class="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center text-xs font-bold text-white">S
        </div>
        <div>
      ...
      <div class="flex-1 py-4 flex flex-col justify-end space-y-3 overflow-hidden text-[11px]">
        ...
        <div v-if="showSenderMessage"
          class="bg-zinc-800 text-zinc-200 p-2.5 rounded-2xl rounded-tl-none max-w-[85%] self-start shadow-sm animate-slide-up">
          Halo, Permisi mau tanya alamat kantornya dimana ya om?
        </div>

        <div v-if="showSenderTyping"
          class="bg-zinc-800/40 text-zinc-400 py-1.5 px-3 rounded-full max-w-max self-start italic text-[10px] flex items-center space-x-1">
          <span class="w-1.5 h-1.5 bg-zinc-400 rounded-full animate-bounce delay-75"></span>
          <span>sedang mengetik...</span>
        </div>

        <div v-if="showBotMessage"
          class="bg-emerald-700 text-white p-2.5 rounded-2xl rounded-tr-none max-w-[85%] self-end shadow-md animate-slide-up">
          Halo !. Kantor kami beralamat di Jl. Merdeka No. 45, Jakarta Pusat. Ada lagi yang bisa dibantu?
        </div>

        <div v-if="showBotTyping"
          class="bg-emerald-900/30 text-emerald-400 py-1.5 px-3 rounded-full max-w-max self-end italic text-[10px] flex items-center space-x-1">
